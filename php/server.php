...
if (isset($_POST['reg_user'])) {
  ...
  // Get max KundenNr and give new user incremented KundenNr
  $getMaxKdNr = "SELECT max(KundenNr) FROM Kunden";
  $KdStatement = $db_conn->prepare($getMaxKdNr);

  $KdStatement->execute();
  $KdResult = $KdStatement->fetch(PDO::FETCH_ASSOC);
  ...
  if (count($errors) == 0) {
    ...
    $query = "INSERT INTO Kunden (KundenNr, Firma, Nachname, Vorname, Telefon, Strasse, Ort, PLZ, Land, Kundenbetreuer, Kreditlimit)
	            VALUES (:newKdNr, :firma,:nachname,:vorname,'0123456789','defaultStr', 'defaultOrt', '90413', 'defaultLand', 1501, 0)";

	  echo $query;

    $statement = $db_conn->prepare($query);
    $statement->bindValue(':newKdNr', $newKdNr);
...
//   if ($user === false) 
//   {
//   	array_push($errors, "KundenNr is required");
//   }
  // if (empty($password)) 
  // {
